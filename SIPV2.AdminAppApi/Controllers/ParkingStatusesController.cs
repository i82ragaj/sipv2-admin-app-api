...
public class ParkingStatusesController : ControllerBase
{
    ...
    private static ParkingStatusDto ToDto(MdparkingStatus status) => new()
    {
        ...
        ParkingActive = status.IdNavigation?.Active,
        ParkingName = status.IdNavigation?.Name,
        ParkingType = status.IdNavigation?.Type,
        ParkingDacode = status.IdNavigation?.Dacode,
        ParkingServerIp = status.IdNavigation?.ServerIp,
        ParkingJob = status.IdNavigation?.Job,
        ParkingLoadDate = status.IdNavigation?.LoadDate,
    };
}
